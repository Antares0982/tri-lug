...
- QQ images: the relay on machine B fetches the bytes (preferring the url provided by NapCat, falling back to the local file from `get_image`), inlines them as base64 into the image segment of `qq.event`, and delivers them over RabbitMQ; alice decodes to bytes and uploads directly. A segment that reaches alice with no `base64` yields **no attachment at all** — there is deliberately no url/file fallback, because the relay only leaves `base64` off after its own GET of that very url has already failed, so forwarding the url would hand the target adapter a url that is known-dead (TG answers `BadRequest: Failed to get http url content`, which fails the *whole* message, text included). An image-only message therefore takes the ordinary log-only path, exactly like voice below.
...
- Must correctly handle all combinations of image and text (including linked text) mixed together, multiple images in one message, etc. The normalized model uses `text:str + ordered attachments[]`: on inbound, merge all text and collect all images (the exact interleaving of text/images is not preserved). On outbound rendering:
  ...
  - TG: images are walked in source order and split by which sender each one needs — `photo`, `animation` (see below) or `document` (see below). A run of stills uses `send_photo` when alone, or a `send_media_group` album in batches of 10; an animated or oversized image goes out alone.
  - Matrix: send one text event first (only if there is text), then one event per image.
  - Reply mapping: `adapter.send` returns **every** native id it produced and the Router links all of them into the same logical message, so a reply pointing at any part resolves. The first returned id is the reply anchor. Whichever leg goes out **first** owns the caption and the `reply_to_message_id`; every later leg is bare. "First" is tracked as an explicit flag, not inferred from "no ids yet" — a leg can be delivered and still return no id (see the timeout rule below), and the next leg must not then repeat the header and the reply.
- **TG images over 10 MB go out as documents.** `sendPhoto` refuses an upload past that outright (`File of size N bytes is too big for a photo`), which used to lose the message. `send_document` delivers the same bytes un-re-encoded, as a file bubble. Two triggers: the byte length up front, and — for a url-only attachment, whose size alice never sees, or a rejection on *dimensions* rather than size — a `BadRequest` from `sendPhoto` that names the photo specifically, which is retried as a document. Any other `BadRequest` (a stale reply target, a lost chat) keeps propagating. Past the Bot API's 50 MB upload ceiling the image is dropped with a WARNING; nothing can be sent. An animated GIF is *not* subject to the photo cap — `sendAnimation` takes 50 MB — so it keeps its motion.
- **Outbound TG media calls use long timeouts, and a timeout is a WARNING, not a delivery failure.** python-telegram-bot's 5s default is a read timeout on Telegram's *reply*, which for a media upload only arrives after Telegram has ingested and (for a GIF) re-encoded the file; it therefore fired routinely on pictures that had in fact been delivered, and the Router logged a delivery-failed traceback for each. Media sends now pass 60s read/write and 20s connect. A `TimedOut` that still gets through is caught per leg: no retry (§1 — a retry would double-post), one WARNING, and the leg contributes no id, so that message simply can't be a reply target from another platform. The remaining legs of a multi-image message still go out.
- **Animated images -> TG must use `send_animation`.** `sendPhoto` re-encodes its input into a static JPEG, so a GIF pushed through it arrives frozen. Two conditions gate the choice:
  ...
  - The upload is always named **`animation.gif`**, never the source's filename. Telegram picks the media type off the extension, and the source rarely carries a usable one — NapCat names files after a content hash (`<hash>.image`, sometimes no extension), and a Matrix event may only offer `body`. Anything but `.gif`/`.mp4` makes `sendAnimation` fall back to a document, i.e. the animation arrives as a **file bubble**. Telegram converts the GIF to MP4 itself and may still decline (small GIFs in particular); that case is delivered-but-as-a-file and logs a WARNING (`send_animation produced a non-animation message`), since nothing else would surface it. The line carries the GIF's geometry and frame count (`media.describe_gif`), which is what makes a pattern in those refusals identifiable — the decision is Telegram's and there is nothing to fix on this side unless the refusals turn out to share a shape, in which case the fix is the same outbound ffmpeg hop noted above (GIF -> MP4 locally, so Telegram has nothing to decline).
