...
- **Animated images -> TG must use `send_animation`.** `sendPhoto` re-encodes its input into a static JPEG, so a GIF pushed through it arrives frozen. Two conditions gate the choice:
  ...
  - The upload is always named **`animation.gif`**, never the source's filename. Telegram picks the media type off the extension, and the source rarely carries a usable one — NapCat names files after a content hash (`<hash>.image`, sometimes no extension), and a Matrix event may only offer `body`. Anything but `.gif`/`.mp4` makes `sendAnimation` fall back to a document, i.e. the animation arrives as a **file bubble**. Telegram converts the GIF to MP4 itself and may still decline; that case is delivered-but-as-a-file and logs a WARNING (`send_animation produced a non-animation message`), since nothing else would surface it.
